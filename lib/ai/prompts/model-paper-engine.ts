...
//
// Implementation note for whoever wires this into the app: the "medium"
// field on generated Paper I/II items should match this app's own `Medium`
// type from lib/types.ts ("sinhala" | "english" | "tamil"). A bilingual run
// (ModelPaperMedium "both") should produce two separate items per question —
// one per medium — sharing a `pairId` or equivalent, not a single item with
// two languages crammed in, since `Question.medium` in lib/types.ts is
// single-valued. Also: the "QuestionSource" union in lib/types.ts
// ("past_paper" | "original" | "command_word_drill") has no AI-generated
// variant yet — add one (e.g. "ai_predicted") before writing these into the
// real question bank, and load them with active: false until a teacher has
// reviewed the batch, matching the "teacher reviews before publishing, never
// auto-publish" rule already in docs/PLAN.md for auto-quiz generation. The
// Paper II schema below has no existing counterpart type in this app —
// define one fresh alongside Question/MockExam in lib/types.ts before
// wiring it up.
...
export interface ModelPaperEngineInput {
  ...
  paper2: {
    ...
    partA: { questionCount: number; mandatory: true };
    partB: { questionCount: number; chooseCount: number };
  };
}
...
2. Never invent a past paper's content. If a year is missing from what was supplied, say so explicitly rather than filling the gap from memory or assumption. The same applies to syllabus structure: if the syllabus text below does not state a unit's teaching-period (hours) allocation, say so explicitly in Step 2 and skip the syllabus-weight ratio for that unit rather than estimating it.
...
6. Respect the exam's real structure exactly: Paper I is ${input.paper1.questions} MCQs in ${input.paper1.durationMinutes} minutes; Paper II is ${input.paper2.durationMinutes} minutes with Part A (${input.paper2.partA.questionCount} structured questions, answer all) and Part B (${input.paper2.partB.questionCount} essay questions, answer ${input.paper2.partB.chooseCount}). Do not invent a different format.
7. ${mediumInstruction} When writing in Sinhala or Tamil, use everyday spoken register and keep technical terms that students and past papers actually use in English (in Latin script) rather than a coined equivalent nobody uses.
...
Combine Steps 2 and 3 into a ranked list of topics/units by confidence band (High / Medium / Low), each with a one- or two-line rationale citing specific evidence, e.g. "asked in 4 of the last 6 years, most recently in [year]; syllabus allocates 12 periods versus an 8% historical share — underrepresented; [source] confirms no syllabus change to this unit." Do not rank more than the top third of units as High (round up; never fewer than 2 units when there are more than 4 in total) — if everything is High, the ranking has failed at its one job.
...
JSON SCHEMA — Paper II / structured & essay items:
{
  "part": "A" | "B",
  "topic": string,
  "medium": "sinhala" | "tamil" | "english",
