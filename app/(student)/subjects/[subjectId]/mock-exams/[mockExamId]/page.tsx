  ...
  return (
    <div className="mt-6">
      ...
      <div className="mt-4 grid grid-cols-3 gap-3 text-center">
        ...
        <Stat label="Rank" value={`${result.rank}/${result.totalAttempts}`} />
      ...
      <ul className="mt-3 space-y-2">
        {Object.entries(result.topicBreakdown).map(([topic, s]) => (
          <li key={topic} className="flex items-center justify-between gap-3 rounded-lg border border-(--color-awaken-line) bg-(--color-awaken-card) px-4 py-2.5 text-sm">
            <span>{topic}</span>
            <span className="text-(--color-awaken-ink-soft)">{s.correct}/{s.total}</span>
          </li>
        ))}
      </ul>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-(--color-awaken-line) bg-(--color-awaken-card) py-3">
