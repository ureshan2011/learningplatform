...
import { PageHeader, Card, CardLink, SectionHeading, Badge } from "@/components/ds";
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-5 py-12">
        ...
        <section lang="si" className="si mt-8 rounded-ict-card border border-ict-orange-500/30 bg-ict-orange-500/12 p-6">
          <h2 className="font-display text-xl font-extrabold text-ict-fg">
            A/L ICT නැවත කරන අය සඳහා — සිංහලෙන්
          </h2>
          <ul className="mt-4 space-y-2.5 text-sm text-ict-fg">
            <li>
          ...
          <p className="mt-4 text-sm">
            <Link href="/go?do=trial&subject=al-ict" className="font-semibold text-ict-accent-fg underline">
          ...
          <ul className="mt-4 space-y-3">
            <li>
              <Card radius="card" className="p-4">
                <p className="font-semibold text-ict-fg">Have a recent paper or mark?</p>
                <p className="mt-1 text-sm text-ict-fg-soft">
                  Trace every lost mark back to its syllabus unit —{" "}
                  <Link href="/past-papers#official" className="text-ict-accent-fg underline">
            ...
            <li>
              <Card radius="card" className="p-4">
                <p className="font-semibold text-ict-fg">Don&apos;t have one, or it&apos;s old?</p>
                <p className="mt-1 text-sm text-ict-fg-soft">
                  Sit{" "}
                  <Link href="/papers/al-ict-2026-paper-1-mcq" className="text-ict-accent-fg underline">
              ...
              <li key={unit.id}>
                <CardLink href={`/syllabus/al-ict/${unit.id}`} radius="card" className="flex flex-wrap items-center justify-between gap-2 p-4">
                  <span className="flex items-baseline gap-3">
                    <span className="font-mono text-sm text-ict-fg-mute">{i + 1}</span>
            ...
            {STAGES.map((stage) => (
              <Card key={stage.window} radius="card" className="p-5">
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <h3 className="font-display font-bold text-ict-fg">{stage.headline}</h3>
                  <Badge tone="brand">{stage.window}</Badge>
                </div>
                <ul className="mt-3 space-y-1.5 text-sm text-ict-fg-soft">
                  {stage.steps.map((s) => (
        ...
        <section className="mt-12">
          ...
          <p className="mt-2 text-ict-fg-soft">
            ...
            and{" "}
            <Link href="/distinguish-between" className="text-ict-accent-fg underline">
          ...
          <div className="mt-4 space-y-3">
            {FAQ.map((faq) => (
              <Card key={faq.q} radius="card" className="overflow-hidden">
                <details>
                  <summary className="cursor-pointer list-none px-5 py-4 font-semibold text-ict-fg">{faq.q}</summary>
