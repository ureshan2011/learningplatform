...
export default function AlIctPaper1Page() {
  return (
    <>
      ...
      */}
      <section className="bg-ict-surface px-5 pb-16">
        <div className="mx-auto max-w-3xl">
          <details className="group overflow-hidden rounded-ict-card border border-ict-line bg-ict-surface-card">
            <summary className="ict-press flex cursor-pointer list-none items-center justify-between gap-3 px-5 py-4 font-semibold text-ict-fg">
              Read the full paper as text, with every answer (no timer)
              <Icon name="expand_more" className="!text-lg shrink-0 text-ict-fg-mute transition-transform duration-[200ms] group-open:rotate-180" />
            </summary>
            <div className="border-t border-ict-line px-5 py-4">
              <h2 className="font-display text-lg font-extrabold tracking-[-0.02em] text-ict-fg">
                ...
                {AL_ICT_2026_PAPER1.map((q, i) => (
                  <li key={q.id} id={`q${q.id}`}>
                    <p className="font-semibold text-ict-fg">
                      {i + 1}. {q.en.stem}
                      {q.replaced ? (
                        <span className="ml-1.5 font-normal text-ict-fg-mute italic">(replacement question — see disclaimer)</span>
                      ) : null}
                    </p>
                    <ol className="mt-1.5 ml-4 list-decimal space-y-0.5 text-ict-fg-mute">
