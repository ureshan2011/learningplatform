...
import { Badge, ButtonLink, Card, Eyebrow, IconBadge, SectionHeading } from "@/components/ds";
    ...
    <>
      ...
      <main className="bg-ict-surface">
        <section className="mx-auto max-w-3xl px-5 pt-10">
          ...
          <h1 className="mt-2 flex items-center gap-2 font-display text-3xl font-extrabold tracking-[-0.02em] text-ict-fg sm:text-4xl">
            <Icon name="auto_awesome" className="!text-3xl text-ict-orange-500" />
            A/L ICT {PREDICTED_PAPER_EXAM_YEAR_TARGET} Predicted Paper
          </h1>
          <p className="mt-3 text-base text-ict-fg-mute">
        ...
        <section className="mx-auto max-w-3xl px-5 py-8">
          ...
          <ul className="mt-3 space-y-2 text-sm text-ict-fg-mute">
            ...
            <li className="flex items-start gap-2">
              ...
              Reviewed by <Link href="/dr-yasas" className="font-semibold text-ict-fg underline">{TEACHER_NAME}</Link>, PhD in Human Interface
            ...
            {AL_ICT_2027_FOCUS_AREAS.map((f) => (
              <li key={f.competencyNumber}>
                <Card radius="md" className="p-3.5 text-sm">
                  ...
                  <p className="mt-1 text-ict-fg-mute">{f.rationale}</p>
                </Card>
              </li>
            ))}
          </ul>
            ...
            {samples.map((q, i) => (
              <li key={q.id}>
                <Card radius="card" className="p-4">
                  ...
                  {q.code ? (
                    <pre className="mt-2 overflow-x-auto rounded-ict-sm bg-ict-surface p-2 font-mono text-xs whitespace-pre">{q.code}</pre>
                  ) : null}
                  <ol className="mt-2 ml-4 list-decimal space-y-0.5 text-sm text-ict-fg-mute">
                    {q.en.options.map((opt, j) => <li key={j}>{opt}</li>)}
                  </ol>
                  <p className="mt-2 text-xs text-ict-fg-mute">{q.topic}</p>
                </Card>
              </li>
            ))}
            <li className="flex flex-col items-center gap-2 rounded-ict-card border border-dashed border-ict-line bg-ict-surface-card p-5 text-center text-sm text-ict-fg-mute">
