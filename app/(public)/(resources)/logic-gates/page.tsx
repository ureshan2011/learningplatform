...
import { PageHeader, Card, SectionHeading } from "@/components/ds";
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-5 py-12">
        ...
        />
        <p className="si mt-2 text-sm text-ict-fg" lang="si">
        ...
        <p className="mt-3 text-sm text-ict-fg-soft">
          ...
          <Link href="/number-systems" className="text-ict-accent-fg underline">
            ...
            <li key={gate.type} id={gate.type} className="scroll-mt-20">
              <Card radius="card" className="p-5">
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <h2 className="text-lg font-bold text-ict-fg">{gate.type} gate</h2>
                  <span className="font-mono text-sm font-semibold text-ict-accent-fg">{gate.expression}</span>
                </div>
                <p className="si mt-1 text-sm text-ict-fg" lang="si">
                  {gate.sinhala}
                </p>
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-[220px_1fr]">
                  ...
                  <div>
                    <table className="w-full border-collapse text-sm">
                      <thead>
                        <tr className="border-b border-ict-line text-left text-xs text-ict-fg-mute uppercase">
                      ...
                      <tbody>
                        {truthRows(gate.double).map((row, i) => {
                          ...
                          return (
                            <tr key={i} className="border-b border-ict-line last:border-0">
                              <td className="py-1.5 pr-4 font-mono">{row.a}</td>
                              {gate.double ? <td className="py-1.5 pr-4 font-mono">{row.b}</td> : null}
                              <td
                                className={
                                  "py-1.5 font-mono font-bold " + (q === 1 ? "text-ict-green-500" : "text-ict-fg-mute")
                                }
                              >
                                {q}
                              </td>
                ...
                <p className="mt-4 text-sm text-ict-fg-soft">{gate.meaning}</p>
                <p className="mt-2 flex items-start gap-2 rounded-ict-md bg-ict-orange-500/12 p-3 text-sm text-ict-accent-fg">
        ...
        <section className="mt-14">
          ...
          <p className="mt-3 text-ict-fg-soft">
                  ...
                  <tr key={i} className="border-b border-ict-line last:border-0">
                    ...
                    <td className="py-1.5">
                      <Icon
                        ...
                        className={"!text-base " + (row.match ? "text-ict-green-500" : "text-ict-danger-fg")}
                      />
                    </td>
                  </tr>
                ))}
