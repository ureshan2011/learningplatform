...
import { Badge, Card, IconBadge, PageHeader, SectionHeading } from "@/components/ds";
        ...
        <div className="mx-auto max-w-3xl px-5 py-12">
          ...
          <Card radius="card" className="mt-6 flex items-start gap-3 p-4">
            ...
            <p className="text-sm text-ict-fg-soft">
              ...
              <a href="#official" className="font-semibold text-ict-accent-fg underline underline-offset-2">
            ...
            <Card radius="card" className="p-6">
              ...
              <p className="mt-4 text-sm">
                <Link
                  ...
                  className="font-semibold text-ict-accent-fg underline underline-offset-2"
                >
          ...
          <section className="mt-12">
            ...
            <Link href="/papers/al-ict-2026-paper-1-mcq" className="mt-4 block">
              <Card variant="feature" radius="card" hoverable className="p-5">
          ...
          {papers.length > 0 ? (
            <section className="mt-12">
              ...
              <p className="mt-2 text-sm text-ict-fg-mute">Free, no sign-up needed.</p>
              <ul className="mt-4 space-y-2">
                {papers.map((item) => (
                  <li key={item.id}>
                    <a
                      href={item.downloadUrl}
                      className="ict-lift flex items-center gap-3 rounded-ict-card border border-ict-line bg-ict-surface-card p-4 shadow-ict-sm"
                    >
                      <IconBadge
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="block font-semibold text-ict-fg">{item.title}</span>
                        <span className="block text-xs text-ict-fg-mute">
                          {KIND_LABEL[item.kind] ?? item.kind} · {formatDate(item.createdAt)}
                        </span>
                      </span>
                      <Icon name="download" className="!text-lg text-ict-fg-mute" />
                    </a>
                  </li>
                ))}
              </ul>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {[EXAM_STRUCTURE.paper1, EXAM_STRUCTURE.paper2].map((paper) => (
                <Card key={paper.name} radius="card" className="p-5">
                  <h3 className="font-display font-bold text-ict-fg">{paper.name}</h3>
                  <p className="mt-1 text-sm font-semibold text-ict-accent-fg">
                    {paper.durationMinutes / 60} hours
                    {"questions" in paper ? ` · ${paper.questions} questions` : ""}
                  </p>
                  <p className="mt-2 text-sm text-ict-fg-soft">{paper.note}</p>
                </Card>
              ))}
            </div>
          </section>
          ...
          <section className="mt-12">
              ...
              {METHOD.map((m) => (
                <li key={m.step}>
                  <Card radius="card" className="flex gap-4 p-5">
                    <span className="grid size-8 shrink-0 place-items-center rounded-full bg-ict-orange-500/12 font-mono text-sm font-bold text-ict-accent-fg">
                      {m.step}
                    </span>
                    <div className="min-w-0">
                      <h3 className="font-display font-bold text-ict-fg">{m.title}</h3>
                      <p className="mt-2 text-sm text-ict-fg-soft">{m.body}</p>
                    </div>
                  </Card>
                </li>
              ))}
            ...
            <p className="mt-4 text-sm text-ict-fg-soft">
              ...
              <Link href="/command-words" className="font-semibold text-ict-accent-fg underline underline-offset-2">
              ...
              time,{" "}
              <Link href="/revision-plan" className="font-semibold text-ict-accent-fg underline underline-offset-2">
                this revision plan
              </Link>{" "}
            ...
            <div className="mt-5 space-y-2">
              {AL_ICT_UNITS.map((unit) => {
                ...
                return (
                  <details
                    key={unit.id}
                    className="rounded-ict-card border border-ict-line bg-ict-surface-card shadow-ict-sm"
                  >
                    <summary className="flex cursor-pointer list-none flex-wrap items-baseline justify-between gap-2 px-5 py-4">
                      <span className="font-semibold text-ict-fg">
                        <span className="text-ict-accent-fg">Unit {unit.competencyNumber}.</span> {unit.title}
                      </span>
                      <span className="text-xs text-ict-fg-mute">
                        Grade {unit.gradeYear} · {unit.periods} periods
                      </span>
                    </summary>
                    <div className="border-t border-ict-line px-5 py-4">
                      <p className="text-sm text-ict-fg-soft">{unit.competencyStatement}.</p>
                      {asked.length > 0 ? (
                        <>
                          <p className="mt-3 text-xs font-semibold tracking-wide text-ict-fg uppercase">
                            Commonly asked as
                          </p>
                      ...
                      <Link
                        ...
                        className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-ict-accent-fg"
                      >
                        Full unit breakdown
                        <Icon name="chevron_right" className="!text-base" />
                      </Link>
              ...
              {OFFICIAL_SOURCES.map((src) => (
                <li key={src.url}>
                  <a
                    ...
                    className="ict-lift block rounded-ict-card border border-ict-line bg-ict-surface-card p-4 shadow-ict-sm"
                  >
                    <span className="flex items-center gap-1.5 font-semibold text-ict-fg">
                      {src.name}
                      <Icon name="north_east" className="!text-sm text-ict-fg-mute" />
                    </span>
                    <span className="mt-1 block text-sm text-ict-fg-soft">{src.note}</span>
                  </a>
                </li>
              ))}
            </ul>
            ...
            <div className="mt-4 space-y-3">
              {FAQS.map((faq) => (
                <details
                  ...
                  className="rounded-ict-card border border-ict-line bg-ict-surface-card shadow-ict-sm"
                >
                  <summary className="cursor-pointer list-none px-5 py-4 font-semibold text-ict-fg">
                    {faq.q}
                  </summary>
                  <p className="border-t border-ict-line px-5 py-4 text-sm text-ict-fg-soft">{faq.a}</p>
                </details>
              ))}
            </div>
          </section>
