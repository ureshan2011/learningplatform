...
import { PageHeader, Card, SectionHeading, ButtonLink, Badge } from "@/components/ds";
...
export default function ShortNotesPage() {
  ...
  return (
    <>
      ...
      />
      <main className="bg-ict-surface print:bg-white">
        <div className="mx-auto max-w-3xl px-5 py-12 print:px-0 print:py-4">
          ...
          <p className="mt-8 hidden text-xs text-ict-fg-mute print:block">
                ...
                {units.map((unit) => (
                  <Card key={unit.id} radius="card" className="break-inside-avoid p-5 print:border print:shadow-none">
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      <h3 className="font-display font-bold text-ict-fg">
                    ...
                    <div className="mt-3 space-y-3">
                      {unit.lessons.map((lesson) => (
                        <div key={lesson.id}>
                          <p className="text-xs font-semibold tracking-wide text-ict-fg-mute uppercase">
                            {lesson.id} {lesson.title}
                          </p>
                          <ul className="mt-1.5 space-y-1">
                            {lesson.examObjectives.map((objective, i) => (
                              <li key={i} className="flex items-start gap-2 text-sm text-ict-fg">
                                <Icon
                                  name="check_circle"
                                  className="mt-0.5 !text-sm shrink-0 text-ict-green-500 print:hidden"
                                />
                                <span className="hidden text-ict-fg-soft print:inline">·</span>
                                {objective}
                              </li>
                            ))}
                          </ul>
                    ...
                    <Link
                      ...
                      className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-ict-accent-fg print:hidden"
                    >
                      Full unit breakdown
