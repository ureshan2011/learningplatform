  ...
  return (
    <main className="mx-auto max-w-[900px] px-4 py-5 sm:px-6 sm:py-6">
      ...
      <h1 className="mt-4 flex items-center gap-2 text-2xl font-bold">
        <Icon name="auto_awesome" className="text-ict-accent-fg" />
      ...
      <section className="mt-8 rounded-ict-md border border-ict-line bg-ict-surface p-4 text-sm">
        <p className="font-semibold">Mandatory framing statement (shown to every student, always)</p>
        <p className="mt-1.5 text-ict-fg-soft">{PREDICTED_PAPER_FRAMING_EN}</p>
      </section>
      ...
      <section className="mt-8">
        ...
        <ul className="mt-3 space-y-2">
          {AL_ICT_2027_FOCUS_AREAS.map((f) => (
            <li key={f.competencyNumber} className="rounded-ict-md border border-ict-line bg-ict-surface-card p-3.5 text-sm">
      ...
      <section className="mt-10">
        ...
        <ol className="mt-3 space-y-4">
          {AL_ICT_2027_PREDICTED_PAPER1.map((q, i) => (
            <li key={q.id} className="rounded-ict-md border border-ict-line bg-ict-surface-card p-4 text-sm">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <p className="font-semibold">
                  {i + 1}. {q.en.stem}
                </p>
              ...
              <p className="mt-1 text-ict-fg-soft">{q.si.stem}</p>
              {q.code ? (
                <pre className="mt-2 overflow-x-auto rounded-ict-md bg-ict-surface p-2 font-mono text-xs whitespace-pre">{q.code}</pre>
              ) : null}
              <ol className="mt-2 ml-4 list-decimal space-y-0.5">
                {q.en.options.map((opt, j) => (
                  <li key={j} className={j === q.correctIndex ? "font-semibold text-ict-green-500" : undefined}>
                    {opt}
                    {q.si.options[j] !== opt ? <span className="text-ict-fg-soft"> / {q.si.options[j]}</span> : null}
                    {j === q.correctIndex ? " ✓" : ""}
                  </li>
                ))}
              </ol>
      ...
      <section className="mt-10 pb-4">
        <h2 className="flex items-center gap-2 text-lg font-semibold">
          <Icon name="edit_note" className="text-ict-accent-fg" />
          Paper II — structured & essay
        </h2>
        <ol className="mt-3 space-y-4">
          {AL_ICT_2027_PREDICTED_PAPER2.map((item) => (
            <li key={item.id} className="rounded-ict-md border border-ict-line bg-ict-surface-card p-4 text-sm">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <p className="font-semibold">
              ...
              {item.en.scenario ? (
                <>
                  ...
                  {item.si.scenario ? <p className="text-ict-fg-soft">{item.si.scenario}</p> : null}
                </>
              ) : null}
              <ol className="mt-2 space-y-1.5">
                {item.subparts.map((sp) => (
                  <li key={sp.label}>
                    ...
                    {sp.si ? <span className="block text-ict-fg-soft">{sp.si}</span> : null}
                  </li>
                ))}
              </ol>
              <p className="mt-2 text-xs text-ict-fg-soft">Mark scheme: {item.markScheme}</p>
              <p className="mt-1 text-xs text-ict-fg-soft">{item.rationale}</p>
            </li>
          ))}
        </ol>
      </section>
...
function BandBadge({ band }: { band: "high" | "medium" | "low" }) {
  const tone =
    band === "high"
      ...
      : band === "medium"
        ? "bg-ict-surface-raised text-ict-accent-fg"
