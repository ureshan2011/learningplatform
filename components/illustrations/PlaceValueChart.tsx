...
export function PlaceValueChart({ bits }: { bits: string }) {
  ...
  return (
    <div className="not-prose overflow-x-auto">
      <div className="inline-flex min-w-full items-end justify-center gap-1.5 rounded-ict-card border border-ict-line bg-ict-surface-card p-4">
        {digits.map((d, i) => {
          const place = 2 ** (width - 1 - i);
          return (
            <div key={i} className="flex flex-col items-center gap-1.5">
              <span className="text-[11px] font-semibold text-ict-fg-mute">{place}</span>
              <div
                className={
                  "flex size-10 items-center justify-center rounded-ict-md border-2 text-lg font-bold sm:size-12 " +
                  (d === 1
                    ? "border-ict-orange-500 bg-ict-orange-500/12 text-ict-accent-fg"
