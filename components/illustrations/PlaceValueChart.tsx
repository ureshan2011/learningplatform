...
export function PlaceValueChart({ bits }: { bits: string }) {
  ...
  return (
    <div className="not-prose overflow-x-auto">
      <div className="inline-flex min-w-full items-end justify-center gap-1.5 rounded-ict-card border border-ict-paper-300 bg-ict-paper-0 p-4">
        {digits.map((d, i) => {
          const place = 2 ** (width - 1 - i);
          return (
            <div key={i} className="flex flex-col items-center gap-1.5">
              <span className="text-[11px] font-semibold text-ict-ink-400">{place}</span>
              <div
                className={
