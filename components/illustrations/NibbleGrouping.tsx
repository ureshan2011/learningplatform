...
export function NibbleGrouping({ bits }: { bits: string }) {
  ...
  return (
    <div className="not-prose overflow-x-auto">
      <div className="inline-flex min-w-full flex-wrap items-start justify-center gap-3 rounded-ict-card border border-ict-line bg-ict-surface-card p-4">
          ...
          <div key={i} className="flex flex-col items-center gap-1.5">
            <div className="flex gap-1">
              {nibble.split("").map((d, j) => (
                <div
                  key={j}
                  className="flex size-9 items-center justify-center rounded-ict-md border-2 border-ict-line text-base font-bold sm:size-10"
                >
                  {d}
                </div>
              ))}
            </div>
            <span className="text-ict-fg-mute">↓</span>
            <div className="flex size-9 items-center justify-center rounded-ict-md border-2 border-ict-orange-500 bg-ict-orange-500/12 text-base font-bold text-ict-accent-fg sm:size-10">
              {parseInt(nibble, 2).toString(16).toUpperCase()}
            </div>
          </div>
        ))}
      </div>
      <p className="mt-3 text-center text-sm text-ict-fg-mute">
        Read the hex digits left to right:{" "}
        <span className="font-semibold text-ict-fg">0x{hex}</span>
      </p>
    </div>
  );
}
