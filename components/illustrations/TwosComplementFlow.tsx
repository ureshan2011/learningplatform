...
export function TwosComplementFlow({ bits }: { bits: string }) {
  ...
  return (
    <div className="not-prose">
      <div className="flex flex-col items-center gap-2 rounded-ict-card border border-ict-line bg-ict-surface-card p-4">
        {steps.map((step, i) => (
          <div key={step.label} className="flex w-full flex-col items-center">
            <div className="flex flex-col items-center gap-1.5">
              <span className="text-[11px] font-semibold text-ict-fg-mute uppercase tracking-wide">
                {step.label}
              </span>
              <Bits digits={step.digits} tone={step.accent ? "accent" : "neutral"} />
            </div>
            {i < steps.length - 1 ? (
              <span className="my-1 text-xl text-ict-fg-mute" aria-hidden>
                ↓
              </span>
            ) : null}
          </div>
